<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" :inline="true" v-show="showSearch" label-width="68px">
      <el-form-item label="设备归属名称" prop="name">
        <el-input v-model="queryParams.name" placeholder="请输入设备归属名称" clearable size="small"
    ...
    <el-table v-loading="loading" :data="listList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="设备归属名称" align="center" prop="name" />
      <el-table-column label="厂商类型" align="center" prop="factoryType">
        <template slot-scope="scope">{{scope.row.factoryType}}</template>
     </el-table-column>
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="设备归属名称" prop="name">
          <el-input v-model="form.name" placeholder="请输入设备归属名称" />
        </el-form-item>
        <el-form-item label="设备编号" prop="no">
          <el-input v-model="form.no" placeholder="请输入设备编号" />
        </el-form-item>
        <el-form-item label="厂商类型" prop="factoryType">
          <el-input v-model="form.factoryType" placeholder="请输入厂商类型" />
        </el-form-item>
      </el-form>
...
<script>
import { listList, getList, delList, addList, updateList, exportList } from "@/api/front/checkwork";
...
/** 提交按钮（数据权限） */
    ...
    handleDelete(row) {
      ...
      this.$confirm('是否确认删除考勤主键为"' + ids + '"的数据项?', "警告", {
